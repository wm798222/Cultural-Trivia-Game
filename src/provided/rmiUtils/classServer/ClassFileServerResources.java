...
//	private String classpath;
	 ...
	 */
	public ClassFileServerResources(int port, String classpath, Consumer<String> infoMsgStrategy, Consumer<String> errMsgStrategy) throws IOException
	{
...
//		this.classpath = classpath;
	 ...
	 */
	public byte[] getBytes(PathSuffix pathSuffix) throws IOException, ClassNotFoundException  {
//	public byte[] getBytes(String path) throws IOException, ClassNotFoundException  {

		showInfoMsg("[ClassFileServerResources.getBytes()] reading on path = " + pathSuffix.path+pathSuffix.suffix);
		
		String filename = URL_SEPARATOR+ pathSuffix.path.replace('.', URL_SEPARATOR) + pathSuffix.suffix;
		showInfoMsg("[ClassFileServerResources.getBytes()] Reading filename = "+filename);
		try {
			...
			String errMsg = "[ClassFileServerResources.getBytes()] filename = "+filename+": Exception while reading file contents: "+e;
...
//		}
	...
	public static void main(String args[]) 
	{
		...
		try {
			new ClassFileServerResources(port, classpath);
		} catch (IOException e) {
			System.err.println("[ClassFileServer.main()] Unable to start ClassServer: " + e.getMessage());
			e.printStackTrace();
		}
	}
}
