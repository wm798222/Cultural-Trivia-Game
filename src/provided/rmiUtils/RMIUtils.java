...
public class RMIUtils implements IRMIUtils {
	 ...
	 * System.err.println()
	 */	
	private Consumer<String> errMsgStrategy = (c) -> {};		
	...
	public Registry getLocalRegistry() {
		...
		try {
			...
			showInfoMsg("[RMIUtils.getLocalRegistry()] Successfully created the local Registry: "+registry);
			return registry;
...
//	private ClassFileServer classFileServer = null;
	...
	private void startClassFileServer(int classServerPort) {
		if (null != classFileServer) {
			showInfoMsg("[RMIUtils.startClassFileServer()] Stopping already running class file server.");
			stopClassFileServer();
		}
...
//					System.getProperty("user.dir"), infoMsgStrategy, errMsgStrategy);
	...
	private void stopClassFileServer() {
		if(null != classFileServer) {
			showInfoMsg("[RMIUtils.stopClassFileServer()] Stopping class file server.");
			classFileServer.stop();
			classFileServer = null;
		}
		else {
			showInfoMsg("[RMIUtils.stopClassFileServer()] No class file server to stop.");
		}
	...
	@Override
	public void stopRMI() {
		showInfoMsg("[RMIUtils.stopRMI()] Stopping RMI...");
		stopClassFileServer();
	}
